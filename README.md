...
4. 上传资料时选择类别、已有产品（或填写新产品）、主险/附加险、条款/费率；文件框支持一次选择多个 Word/PDF，批量时资料名称按文件名生成。
...
- 预览与下载共用下载权限：PDF 在浏览器内嵌预览，DOCX 提取正文和表格预览，旧版 DOC 提示下载原文件；原文件仍保存在私有存储中。系统不提供恶意软件扫描。
